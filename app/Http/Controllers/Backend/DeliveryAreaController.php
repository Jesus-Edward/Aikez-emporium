...
class DeliveryAreaController extends Controller
{
    ...
    public function store(DeliveryAreaCreateRequest $request)
    {
        ...
        $area->save();
        $notification = array('message' => 'Area created successfully', 'alert-type' => 'success');
        return to_route('admin.delivery-area.index')->with($notification);
    }
    ...
    public function update(DeliveryAreaCreateRequest $request, string $id)
    {
        ...
        $area->update();
        $notification = array('message' => 'Area updated successfully', 'alert-type' => 'success');
        return to_route('admin.delivery-area.index')->with($notification);
    }
    ...
    public function destroy(string $id)
    {
        try {
            $area = DeliveryArea::findOrFail($id);
            $table = '#deliveryarea-table';
            $area->delete();
            return response(['status' => 'success', 'message' => 'Area deleted successfully!', 'table' => $table]);
        } catch (\Exception $e) {
            logger($e);
